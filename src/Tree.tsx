...
const [TreeProvider, useTreeContext] =
  createTreeContext<TreeContextValue>(TREE_NAME);
...
const Tree = React.forwardRef<TreeElement, ScopedProps<TreeProps>>(
  (props, forwardedRef) => {
    const {
      __scopeTree,
    ...
    const openKeysSet = React.useMemo(
      () => new Set(openKeysArray),
      [openKeysArray],
    );
...
const TreeItem = React.forwardRef<TreeItemElement, ScopedProps<TreeItemProps>>(
  (props, forwardedRef) => {
    ...
    const { openKeys, onToggleOpen, selectedKeys, onSelect, orientation } =
      useTreeContext(TREE_NAME, __scopeTree);
...
export type FlatNode<T extends RecursiveNode = RecursiveNode> = Omit<
  T,
  "nodes"
> & {
  depth: number;
